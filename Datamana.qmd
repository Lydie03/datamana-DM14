...
### Improving the modelization of pollution - GDP as a function of gas emissions and deaths due to pollution (model4)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
chi2_statistic <- chi2_test$statistic 
p_value <- chi2_test$p.value print(chi2_statistic) print(p_value)
```
...
### Regression model : a Korean environmental Kuznets curve ?
...
ggplot(merged_korea, aes(x = GDP, y = Pollution)) + 
  ...
  labs(title = "Linear Regression of GDP vs. Pollution in South Korea", x = "GDP (Current US$)", y = "Pollution (PM2.5)") + 
  theme_minimal()
```
