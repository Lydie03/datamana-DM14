...
print(summary_table)
```
This summary allow us to highlight major discrepancies between regions. Arranged from highest to lowest DALYs mean, we get that mostly developing countries appear first, prevalent in textile and raw product importations. Lowest are more developed countries, prevalent in services. This will allow us to determine whether this indicates a causality with GDP level and growth.
...
median <- death_air_pollution %>%
  group_by(ParentLocation) %>%
  summarise(median = median(median_value, na.rm = TRUE))

sd <- death_air_pollution %>%
  group_by(ParentLocation) %>%
  summarise(sd = sd(median_value, na.rm = TRUE))

summary_table <- data.frame(
  "Mean of deaths due to pollution" = mean,
  "Median of deaths due to pollution" = median_value,
  "Standard Deviation of deaths due to pollution" = sd
)
