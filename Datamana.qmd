...
# Print the table
kable(summary_table, caption = "Summary Statistics of DALYs")
```
...
# Print the table
table(summary_table, caption = "Summary Statistics of Deaths Due to Pollution per Country")
```
Arranged by the number of deaths due to air pollution, we get first two countries are China, India and China and last three ones St Vincent, Samoa and St Lucia. These observations might require to correct the amount of deaths with the population size.
...
## PIB and gas emission : Kuznets curve (EKC)
### Econometrically 
```{r}
merged_data <- DALYs %>%
  inner_join(GDP_per_capita_cleaned, by = c("Country", "Period")) %>%
  inner_join(death_air_pollution, by = c("Country", "Period"))
```

### Graphical representation 
Now, let's see if we can find the graphical representation of the EKC theory. 

First option : choose one year (2020)
```{r}
# Sélectionner le PIB pour l'année 2020
GDP_per_capita_selected <- GDP_per_capita1 %>%
  select(Country, Mean_GDP = `2020`)

# Fusionner avec les autres datasets
merged_data <- GDP_per_capita_selected %>%
  inner_join(death_air_pollution1, by = "Country") %>%
  inner_join(SDG1, by = "Country") %>%
  inner_join(DALYs1, by = "Country")

# Calculer la régression linéaire entre le PIB et la pollution
model <- lm(Mean_Value ~ Mean_GDP, data = merged_data)

# Afficher le résumé du modèle
summary(model)

```
Second option : mean of GDP over the years 
```{r}
# Mean PIB between 1960 to 2023
GDP_per_capita_selected <- GDP_per_capita1 %>%
  select(Country, matches("^19|^20")) %>%  # Choose all columns from 1960 to 2023
  rowwise() %>%
  mutate(Mean_GDP = mean(c_across(starts_with("19"):starts_with("20")), na.rm = TRUE)) %>%  
  ungroup() %>%
...
# Calculation of the regression between GDP and air pollution 
model <- lm(Mean_Value ~ Mean_GDP, data = merged_data)
