...
```{r, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
chi_squared <- anova_model$"Sum Sq"
df <- anova_model$"Df"
p_value <- pchisq(chi_squared, df, lower.tail = FALSE) 
list(Chi_squared = chi_squared, Degrees_of_freedom = df, P_value = p_value)
```
We obtain a very low p value, indicating there is a significant relationship between GD¨and SDG. 
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
chi_squared <- anova_model3$"Sum Sq"
df <- anova_model3$"Df"
p_value <- pchisq(chi_squared, df, lower.tail = FALSE) 
list(Chi_squared = chi_squared, Degrees_of_freedom = df, P_value = p_value)
```
Our low p value indicates e significant relationship between our explanatories and explained variables. 
#### Analysis
...
It appears our results are completely different for countries depending on where they are on the globe. We could draw the hypothesis that this is due to the nature of its growth - is the country producing services or textile for inetance. 
It consequently seems we cannot obtain a perfect environmental Kuznets curve, and the theory, while interesting, has to be confronted to other factors in order to be perfected. 

## Conclusion

Our goal in this research paper was to examine whether, on a global scale, countries align with the Environmental Kuznets Curve (EKC) theory. This theory, introduced by Grossman and Krueger in the 1990s, posits that countries initially increase pollution as they grow wealthier, but beyond a certain level of economic development, they continue to prosper while reducing pollution. Countries show varied perspectives on GDP, pollution, its limitations, and its impacts.
 
However, due to the challenges of managing large datasets, we were unable to analyze this issue on a global scale and over a long period. We then decided to focus on countries. 

When examining specific countries at different stages of development, such as Finland and South Korea, we can observe parts of the Kuznets curve in action.
...
This echoes the theory of decoupling, presented by Mathilde Salin in 2020, which suggests that "it would be possible to sustain economic growth while reducing or even eliminating the use of resources and the emission of pollutants (i.e., 'decoupling' them from GDP growth) through substitutions and technological progress." 
However, there is a distinction between absolute decoupling (France, for example, reduced its domestic emissions by 20% between 1990 and 2020), and carbon footprint decoupling, meaning the pollution generated by French consumption, which has only stabilized in recent years. Relative decoupling is far less encouraging than absolute decoupling, but it reflects reality: a significant share of polluting industries has been relocated from developed countries.

This question also aligns with the work of Esther Duflo, a Nobel Prize-winning economist, who argued in a *Les Échos* article that wealthier countries should compensate poorer nations for their contribution to pollution and help them adapt to climate change, as these populations are often the most vulnerable.

These articles question which data should be considered to accurately measure the impact of each country : consumption, wealth, populations... parameters are numerous, as we saw in our regression models. 
...
10. Duflo, Esther.
    ...
    *Esther Duflo, Prix Nobel d'économie: Il faut offrir une compensation aux citoyens les plus pauvres du monde et les aider à s'adapter au changement climatique*.
    Tribune publiée dans le journal français *Le Monde*.
