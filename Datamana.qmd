...
new_data$SDG <- predict(model3, newdata = new_data) 

ggplot(your_data, aes(x = deaths_due_to_pollution, y = SDGs, color = factor(Mean_GDP))) + geom_point() + geom_line(data = new_data, aes(y = SDGs), linetype = "solid") + labs(title = "Interaction Effect of Pollution and GDP on SDGs", x = "Deaths Due to Pollution", y = "SDGs") + theme_minimal() + theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5), axis.title = element_text(size = 12, face = "bold"), legend.title = element_text(size = 10, face = "bold"), legend.position = "right") # Display the plot
