...
The intercept has a value of 554.7, which significate that for a GDP equal to zero, the predicted value for SDGs is 554.7. 
The slope coefficient, of -0.02692. It significates that for an increase of 1 of the GDP, we witness a decrease of -0.02692 of the SDGs. 
This number show a negative relation between the GDP and the SDGs, as to when a country become richer, he is less likely to match the sustainable development goals (SDGs). 

Coefficient significativity : 
We witness a t and p-value very low (<2e-16), which mean that the relation between GDP and SDG is not due to randomness. 

Model fit : 
This model overall is not the most appropriate, considering our very low R squared (0,05283). This indicates that other factors influence SDGs. 
We also have 32400 observations deleted due to missingness, which can affect the calculation of the linear regression. 
...
#### Analysis 
Coefficient interpretation : 
The intercept is equal to 4756, which significate that for SDGs and number of deaths due to air pollution equal to zero, the predicted value for GDP is 4756. 

The coefficient for the deaths due to air pollution is -0.009627, an increase of one death due to air pollution lead to a decrease of the GDP of -0.009627. This means that a very weak negative relation between GDP and deaths due to air pollution, which is quite logical, as the death of one person leads to the loss of a potential workforce. 

The coefficient for the SDGs is equal to -1.978. An increase of one unity of the SDGs lead to a decrease of -1.978 of the GDP. This significates that when a country is more respecting the sustainable development goals, its GDP decrease. The interpretation of this is that pollution is leading to more wealth. 

The coefficient for the interaction between the number of deaths due to air pollution and the SDG is 4.479e-06. This interaction is very weak but positive. 

Significativity of the coefficients : 
The p-value is <2e-16 for all coefficients, which means they are highly significant, the relationships between the variables (deaths due to pollution, SDGs, and average GDP) are statistically significant and unlikely to be due to chance.

Model fit : 
This model overall is not the most appropriate, considering our very low R squared (5.56%) and a high residual standard error (5396). This indicates that other factors influence GDP.
...
##### Analysis 
Coefficient interpretation : 
The intercept is equal to 552,8, which means that when the deaths due to air pollution and the GDP are equal to zero, the predicted value of the SDGs is 552.8. 
The coefficient for the deaths due to air pollution is 3.821e-04, an increase of one death due to air pollution lead to an increase of the SDGs of 0.0003821. This means that a very weak positive relation between SDGs and deaths due to air pollution, which is quite counterintuitive as an augmentation of the deaths due to air pollution should come from a decrease of the respect of the SDGs, so more pollution. 
Other factors could explain this relation. 

The coefficient for the mean of GDP is equal to -0.02639. An increase of one unity of the PIB lead to a decrease of -0.02639 of the SDGs. This significates that when a country is becoming richer, it is less respecting the goals of the SDGs, especially in the pollution field. 

The coefficient for the interaction between the number of deaths due to air pollution and the mean of the GDP is -2.58e-07. This interaction is very weak but negative. 
This indicates that the combined effect of deaths due to pollution and average GDP on the SDGs is slightly influenced by the interaction between the two variables, but the effect remains relatively weak. It suggests that there isn't a strong synergy between these two factors in the model.

Coefficient significativity : 
We witness a t and p-value very low (<2e-16), which mean that the relation between GDP, SDG and the number of deaths due to air pollution is not due to randomness. 

Model fit : 
This model overall is not the most appropriate, considering our very low R squared (0.05331). This indicates that other factors influence SDGs.
We also have 32400 observations deleted due to missingness, which can affect the calculation of the linear regression.
...
long_data <- Burkina_Faso_SDG %>% pivot_longer(cols = everything())
                                    ...
                                    # Specify which columns to pivot names_to = "Variable", # Name of the new column for the original column names values_to = "Value" # Name of the new column for the original values )
```
