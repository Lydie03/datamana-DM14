...
write.csv(GDP_per_capita_cleaned, "GDP_per_capita_modified.csv", row.names = FALSE)
```
...
```{r}
library(dplyr)
DALYs <- DALYs |> rename("Country Name" = "Country")
mean_DALYs <- mean_DALYs |> rename("Country Name" = "Country")
death_air_pollution_unique <- death_air_pollution_unique |> rename("Country Name" = "ParentLocation")
mean_SDG <- mean_SDG |> rename("Country Name" = "Country")

Merged_data <-  DALYs %>%
   inner_join(mean_gdp, by = c("Country Name")) %>%
   inner_join(mean_DALYs, by = c("Country Name")) %>%
  inner_join(death_air_pollution_unique, by = c("Country Name"))%>% 
  inner_join(mean_SDG, by = c("Country Name")) #la base finale est vide car les noms des pays ne sont pas les mêmes... 
``` 

### Linear regression between GDP and air pollution (SDG) 
```{r}
model <- lm(Mean_Value.x ~ Mean_GDP, data = Merged_data)
#problem with the values in the merged data base 
```
...
ggplot(Merged_data, aes(x = Mean_GDP, y = Mean_Value.x)) +
  geom_point() +  # Ajoute les points du nuage
  labs(
    x = "GDP",
    y = "SDG" 
  ) +
  theme_minimal()  # au résultat on voit que la base "merged" est vide 
```
### other try : merge with code for each country 
```{r}
# Identifier les codes pays présents dans chaque dataset
common_codes <- Reduce(intersect, list(`GDP_per_capita`$`Country Code`, SDG$SpatialDimValueCode, death_air_pollution$SpatialDimValueCode, DALYs$SpatialDimValueCode))


# Filtrer les bases de données avec les codes communs
GDP_filtered <- GDP_per_capita[GDP_per_capita$`Country Code` %in% common_codes, ]
SDG_filtered <- SDG[SDG$`SpatialDimValueCode` %in% common_codes, ]
death_air_pollution_filtered <- death_air_pollution[death_air_pollution$`SpatialDimValueCode` %in% common_codes, ]
DALYs_filtered <- DALYs[DALYs$`SpatialDimValueCode` %in% common_codes, ]

# Fusionner les bases filtrées
merged_data <- merge(GDP_filtered, SDG_filtered, by.x = "Country Code", by.y = "SpatialDimValueCode", all = TRUE)
merged_data <- merge(merged_data, death_air_pollution_filtered, by.x = "Country Code", by.y = "SpatialDimValueCode", all = TRUE)
merged_data <- merge(merged_data, DALYs_filtered, by.x = "Country Code", by.y = "SpatialDimValueCode", all = TRUE)
